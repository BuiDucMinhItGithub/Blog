...
<title>Sửa thông tin customer</title>
...
<body>
	<p>${message}</p>
	<div class="container">
		<h4>Sửa thông tin customer</h4>
		<c:url value="customer-edit" var="url"></c:url>
		<form:form action="${url}" method="POST" modelAttribute="customer">
			<div class="form-group">
				<label for="id">Mã </label> <br> 
				<form:input class="form-control" path="id" value="${customer.id}" readonly="true"/>
			</div>
			<div class="form-group">
				<label for="name">Tên</label> <br> 
				<form:input class="form-control" path="name" value="${customer.name}"/>
			</div>
			<div class="form-group">
				<label for="state">Trạng thái</label> <br> 
				<form:input class="form-control" path="state" value="${customer.state}"/>
			</div>
